...
class VoyagerAPIIterator(object):

    config = Config()

    def __init__(self, base_url, params=None, result_class=None, page_param="page", next_page_param="next", per_page_param="page_size", per_page=10, results="results"):
        """
        ...
        self.current_page = 1
        self.next_page_param = next_page_param
        self.current_items = []
    ...
    def _fetch_next_page(self):
        ...
        if self.result_class:
            self.current_items = [self.result_class.from_json(f) for f in data.get(self.results, [])]
        else:
            self.current_items = data.get(self.results, [])
        self.current_page += 1
        self.has_more = data.get(self.next_page_param) is not None and self.current_page <= data.get("count")
